...
def plot_rastermap(
    ...
    """
    Plot liquefaction density map using raster clipped to the AOI.
    ...
    cbar.set_label("Liquefaction Susceptibility", rotation=90)


    ax.set_title(f"{city_name} – Liquefaction Susceptibility")
    ax.axis("off")
    ...
    out_path = os.path.join(img_dir, f"{city_name}_liquefaction_map.png")
...
def plot_histogram(
    ...
    ax.set_xlabel("")
    ax.set_ylabel("")
    ax.set_title(f"{city_name} – Liquefaction Susceptibility Distribution Histogram")

    # Save
    img_dir = os.path.join(output_dir, "images")
    os.makedirs(img_dir, exist_ok=True)

    out_path = os.path.join(img_dir, f"{city_name}_liquefaction_histogram.png")
    plt.savefig(out_path, dpi=200, bbox_inches="tight")
    plt.close()
