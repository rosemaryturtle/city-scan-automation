...
def datacollection(
    ...
    solar_blob = f"solar/PVOUT.tif"
    solar_url = bucket_base + solar_blob

    # Rasterio requires /vsicurl/ for HTTP-based TIFFs
    raster_path = f"/vsicurl/{solar_url}"
    logger.info(f"Requesting solar raster: {solar_url}")

    try:
        with rasterio.open(raster_path) as src:
            logger.info("solar raster opened successfully.")
        ...
        logger.error(f"Error reading or clipping solar raster: {e}")
        return None
    ...
    output_path = os.path.join(spatial_dir, f"{city_name}_solar.tif")

    # Save clipped raster
    try:
        with rasterio.open(output_path, "w", **clipped_meta) as dst:
            dst.write(clipped_image)
        logger.info(f"Clipped solar saved to: {output_path}")
    except Exception as e:
        logger.error(f"Error saving clipped raster: {e}")
        return None

    logger.info("solar complete.")
...
def datacollection_monthly(
    ...
    # Construct GCS URL
    bucket_base = "https://storage.googleapis.com/city-scan-global-public/"
    solar_blob = f"solar/PVOUT-monthly.tif"
    solar_url = bucket_base + solar_blob

    # Rasterio requires /vsicurl/ for HTTP-based TIFFs
    raster_path = f"/vsicurl/{solar_url}"
    logger.info(f"Requesting monthly solar raster: {solar_url}")

    try:
        with rasterio.open(raster_path) as src:
            logger.info("monthly solar raster opened successfully.")
        ...
        logger.error(f"Error reading or clipping monthly solar raster: {e}")
        return None
    ...
    output_path = os.path.join(spatial_dir, f"{city_name}_solar_monthly.tif")
    ...
    logger.info("monthly solar complete.")
