...
if menu['burned_area']:  # processing time: 5h
    import burned_area
    burned_area.burned_area(aoi_file, global_inputs['burned_area_blob'], data_bucket, local_data_dir, local_output_dir, city_name_l, cloud_bucket, output_dir)
...
if menu['demographics']:  # processing time: 19m
    ...
    wp_file_json = requests.get(f"https://www.worldpop.org/rest/data/age_structures/ascic_2020?iso3={city_inputs['country_iso3']}").json()
    ...
    with open(f'{local_output_dir}/{city_name_l}_demographics.csv', 'w') as f:
        ...
        for i in [1] + list(range(0, 85, 5)):
            for s in sexes:
                out_image, out_meta = raster_pro.raster_mask_file(f'{local_demo_folder}/{city_inputs["country_iso3"].lower()}_{s}_{i}_2020_constrained.tif', features)
