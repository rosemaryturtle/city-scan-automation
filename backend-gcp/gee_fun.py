...
def gee_landcover(city_name_l, aoi_file, local_output_dir, cloud_bucket, output_dir):
    ...
    task0 = ee.batch.Export.image.toCloudStorage(**{'image': lc_aoi,
                                                    ...
                                                    'scale': 10,
                                                    'bucket': cloud_bucket,
                                                    'fileNamePrefix': f"{output_dir}/{city_name_l}_lc",
                                                    'maxPixels': 1e9,
                                                    'fileFormat': 'GeoTIFF',
                                                    'formatOptions': {
                                                        'cloudOptimized': True,
...
def gee_elevation(city_name_l, aoi_file, cloud_bucket, output_dir):
    ...
    task0 = ee.batch.Export.image.toCloudStorage(**{'image': elevation_clip,
                                                    ...
                                                    'scale': 30,
                                                    'bucket': cloud_bucket,
                                                    'fileNamePrefix': f"{output_dir}/{city_name_l}_elevation",
                                                    'maxPixels': 1e9,
                                                    'fileFormat': 'GeoTIFF',
                                                    'formatOptions': {
                                                        'cloudOptimized': True,
...
def gee_forest(city_name_l, aoi_file, cloud_bucket, output_dir):
    ...
    task0 = ee.batch.Export.image.toCloudStorage(**{'image': forestCover23,
                                                    ...
                                                    'scale': 30,
                                                    'bucket': cloud_bucket,
                                                    'fileNamePrefix': f"{output_dir}/{city_name_l}_forest_cover23",
                                                    'maxPixels': 1e9,
                                                    'fileFormat': 'GeoTIFF',
                                                    'formatOptions': {
                                                        'cloudOptimized': True,
    ...
    task1 = ee.batch.Export.image.toCloudStorage(**{'image': deforestation_year,
                                                    ...
                                                    'scale': 30,
                                                    'bucket': cloud_bucket,
                                                    'fileNamePrefix': f"{output_dir}/{city_name_l}_deforestation",
                                                    'maxPixels': 1e9,
                                                    'fileFormat': 'GeoTIFF',
                                                    'formatOptions': {
                                                        'cloudOptimized': True,
...
def gee_lst(city_name_l, aoi_file, local_output_dir, first_year, last_year, data_bucket, cloud_bucket, output_dir, season):
    ...
    task = ee.batch.Export.image.toCloudStorage(**{
        ...
        'bucket': cloud_bucket,
        'fileNamePrefix': f"{output_dir}/{city_name_l}_{season}",
        'region': AOI,
        'scale': 30,
        'maxPixels': 1e9,
        'fileFormat': 'GeoTIFF',
...
def gee_ndxi(city_name_l, aoi_file, local_output_dir, first_year, last_year, data_bucket, cloud_bucket, output_dir, index_type):
    ...
    task = ee.batch.Export.image.toCloudStorage(**{
        ...
        'bucket': cloud_bucket,
        'fileNamePrefix': f"{output_dir}/{city_name_l}_{index_type}_season",
        'region': AOI,
        'scale': 10,
        'maxPixels': 1e9,
        'fileFormat': 'GeoTIFF',
...
def gee_nightlight(city_name_l, aoi_file, cloud_bucket, output_dir):
    ...
    task0 = ee.batch.Export.image.toCloudStorage(**{
        ...
        'bucket': cloud_bucket,
        'fileNamePrefix': f"{output_dir}/{city_name_l}_linfit",
        'region': AOI,
        'scale': 100,
        'maxPixels': 1e9,
        'fileFormat': 'GeoTIFF',
    ...
    task1 = ee.batch.Export.image.toCloudStorage(**{
       'image': sum_of_light.select('avg_rad_sum'),
        ...
        'bucket': cloud_bucket,
        'fileNamePrefix': f"{output_dir}/{city_name_l}_avg_rad_sum",
        'region': AOI,
        'scale': 100,
        'maxPixels': 1e9,
        'fileFormat': 'GeoTIFF',
