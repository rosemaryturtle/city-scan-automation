...
if menu['flood_stats']:
    ...
    # FLOOD WSF STATS ###################################
    if exists(output_folder / f'{city_name_l}_wsf_utm.tif'):
        # Reproject flood raster -------------------
        with rasterio.open(output_folder / f'{city_name_l}_wsf_utm.tif') as wsf:
            tgt_crs = wsf.crs
        ...
        for ft in flood_types:
            if exists(output_folder / f'{city_name_l}_{ft}_2020_lt1.tif'):
                with rasterio.open(output_folder / f'{city_name_l}_{ft}_2020_lt1.tif') as src:
                    src_crs = src.crs
                ...
                with rasterio.open(
                    output_folder / f'{city_name_l}_{ft}_2020_lt1_utm.tif',
                    'w',
                    driver='GTiff',
                    height=tgt_height,
                    width=tgt_width,
        ...
        for ft in flood_types:
            if exists(output_folder / f'{city_name_l}_{ft}_2020_lt1_utm.tif'):
                with rasterio.open(output_folder / f'{city_name_l}_{ft}_2020_lt1_utm.tif') as src:
        ...
        if len(flood_arrays) >= 2:
            ...
            with rasterio.open(output_folder / f'{city_name_l}_combined_2020_lt1_utm.tif', 'w', **out_meta) as dst:
        ...
        with rasterio.open(output_folder / f'{city_name_l}_wsf_utm.tif') as wsf:
            ...
            for ft in flood_types:
                if exists(output_folder / f'{city_name_l}_{ft}_2020_lt1_utm.tif'):
                    ...
                    with rasterio.open(output_folder / f'{city_name_l}_{ft}_2020_lt1_utm.tif') as fld:
        ...
        flood_df = []
        for ft in flood_types:
            if not flood_stats[ft] is None:
                flood_df.append(pd.DataFrame(list(flood_stats[ft].items()), columns=['year', ft]))

        merged_df = pd.read_csv(output_folder / f'{city_name_l}_wsf_stats.csv')
        ...
        while i < len(flood_df):
            ...
            i += 1
        merged_df.to_csv(output_folder / f'{city_name_l}_built_up_flood_exposure.csv', index = False)
    else:
