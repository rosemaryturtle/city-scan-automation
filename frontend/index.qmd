...
```{r}
#| output: asis
include_html_chart(fuzzy_read(file.path(output_dir, "plots/html"), "pop_growth.html", paste))
```
...
```{r}
#| output: asis
include_html_chart(fuzzy_read(file.path(output_dir, "plots/html"), "pop-density", paste))
```
...
#| include: false
#| output: asis
# include_html_chart(fuzzy_read(file.path(output_dir, "plots/html"), "???", paste))
```

```{r}
#| output: asis
print_slide_text(slide_texts$demographics$population_distribution_age)
```
...
include_html_chart(fuzzy_read(file.path(output_dir, "plots/html"), "age_stats", paste))
```
...
### Cities’ Shares of National Population, Employment & GDP

```{r}
#| output: asis
include_html_chart(fuzzy_read(file.path(output_dir, "plots/html"), "national-shares", paste))
```

```{r}
#| output: asis
print_slide_text(slide_texts$economic_activity$national_shares)
```

### City growth in GDP, employment and population

```{r}
#| output: asis
# Plot width is wrong: makes left side too large
include_html_chart(fuzzy_read(file.path(output_dir, "plots/html"), "oxford-gdp-plot", paste))
```

```{r}
#| output: asis
print_slide_text(slide_texts$economic_activity$gdp_growth)
```

```{r}
#| output: asis
# Plot width is wrong: makes left side too large
include_html_chart(fuzzy_read(file.path(output_dir, "plots/html"), "oxford-emp-plot", paste))
```

```{r}
#| output: asis
print_slide_text(slide_texts$economic_activity$employment_growth)
```

```{r}
#| output: asis
# Plot width is wrong: makes left side too large
include_html_chart(fuzzy_read(file.path(output_dir, "plots/html"), "oxford-pop-plot", paste))
```

```{r}
#| output: asis
print_slide_text(slide_texts$economic_activity$population_growth_multicity)
```
...
```{r}
#| output: asis
print_slide_text(slide_texts$economic_activity$gdp_per_capita_growth)
```
...
#| output: asis
# Plot width is wrong: makes left side too large
include_html_chart(fuzzy_read(file.path(output_dir, "plots/html"), "gva-sectors", paste))
```

```{r}
#| output: asis
print_slide_text(slide_texts$economic_activity$gva_shares)
```
...
```{r}
#| output: asis
include_html_chart(fuzzy_read(file.path(output_dir, "plots/html"), "urban_built_up_area.html", paste))
```
...
```{r}
#| output: asis
include_html_chart(fuzzy_read(file.path(output_dir, "plots/html"), "landcover_treemap", paste))
```
...
```{r}
#| output: asis
include_html_chart(fuzzy_read(file.path(output_dir, "plots/html"), "PV_graph", paste))
```
...
#| output: asis
# Plot width is wrong: makes left side too large
include_html_chart(fuzzy_read(file.path(output_dir, "plots/html"), "flood_timeline", paste))
```

```{r}
#| output: asis
print_slide_text(slide_texts$demographics$flood_events)
```
...
#| output: asis
#| label: fluvial_chart
include_html_chart(fuzzy_read(file.path(output_dir, "plots/html"), "fu_wsf", paste))
```

```{r}
#| output: asis
#| label: fluvial_text
print_slide_text(slide_texts$risk_identification$flooding_infrastructure_fluvial)
# print_slide_text(slide_texts$risk_identification$flooding_builtup_fluvial)
# print_slide_text(slide_texts$risk_identification$flooding_population_fluvial)
```
...
#| output: asis
#| label: pluvial_chart
include_html_chart(fuzzy_read(file.path(output_dir, "plots/html"), "pu_wsf", paste))
```

```{r}
#| output: asis
#| label: pluvial_text
print_slide_text(slide_texts$risk_identification$flooding_infrastructure_pluvial)
# print_slide_text(slide_texts$risk_identification$flooding_builtup_pluvial)
# print_slide_text(slide_texts$risk_identification$flooding_population_pluvial)
```
...
```{r}
#| output: asis
include_html_chart(fuzzy_read(file.path(output_dir, "plots/html"), "comb_wsf", paste))
```

```{r}
#| output: asis
print_slide_text(slide_texts$risk_identification$flooding_infrastructure_combined)
# print_slide_text(slide_texts$risk_identification$flooding_builtup_combined)
# print_slide_text(slide_texts$risk_identification$flooding_population_combined)
```
...
```{r}
#| output: asis
include_html_chart(fuzzy_read(file.path(output_dir, "plots/html"), "elevation_treemap", paste))
```
...
```{r}
#| output: asis
include_html_chart(fuzzy_read(file.path(output_dir, "plots/html"), "slope_treemap", paste))
```
...
```{r}
#| output: asis
include_html_chart(fuzzy_read(file.path(output_dir, "plots/html"), "ear#| thquake_timeline", paste))
```
...
#| include: false
#| output: asis
# include_html_chart(fuzzy_read(file.path(output_dir, "plots/html"), "fwi.html", paste))
```
...
```{r}
#| output: asis
print_md(slide_texts$conclusion$concluding_questions)
```
