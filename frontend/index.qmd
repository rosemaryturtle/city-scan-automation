...
city_text_file <- list.files(file.path(user_input_dir, "text-files"), pattern ="manual-text\\.md$", full.names = TRUE)[1]
...
```{r}
#| output: false
pop_density_file <- fuzzy_read(charts_dir, "oxford-pop-density-scatter", paste)
age_structure_file <- fuzzy_read(charts_dir, "oxford-age-structure-area", paste)
ojs_define(pop_density_plot = pop_density_file, age_structure_plot = age_structure_file)
```
...
loadData = {
  var [pg, pas, rwi_area, uba, uba_area, lc, pug, pv, pv_area,
    ...
    FileAttachment("./02-process-output/tabular/processed/uba_tracker.csv").csv({typed: true}),
    FileAttachment("./02-process-output/tabular/processed/fe.csv").csv({typed: true}), 
    FileAttachment("./02-process-output/tabular/processed/fu_prob.csv").csv({typed: true}),
    FileAttachment("./02-process-output/tabular/processed/pu_prob.csv").csv({typed: true}),
    FileAttachment("./02-process-output/tabular/processed/cu_prob.csv").csv({typed: true}),
  ]);
