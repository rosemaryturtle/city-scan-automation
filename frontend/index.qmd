...
ojs_define(createdMaps = c("population", "rwi", "economic_activity", "economic_change", "wsf", "land_cover", "intersections", "school_zones", "school_points", "health_zones", "health_points", "solar", "air_quality", "summer_lst", "vegetation", "forest", "deforest", "infrastructure", "elevation", "slope", "landslides", "liquefaction", "burnable"))
```
...
function fillSlide(layer, chart = null, extra_layers = null, title = null, slide_text = null) {
  if (!createdMaps.includes(layer)) return md``;
  var mappingLayers = [...(extra_layers ?? []), layer].map(lay => layerParams[lay].group_id).join(";");
  // To avoid rendering "null", "undefined", or "" in the document, return nothing explicitly:
  // if (!createdMaps.includes(layer)) return;
    if (!slide_text) slide_text = slide_texts[layer];
    if (!title) title = slide_text?.title;
    if (!title) title = layer;
return html`
<section class="level3">
${md`### ${title}`}
${chart ?? ""}
${html`<div class="map-list" data-layers="${mappingLayers}"></div>`}
${addSlideText(slide_texts[layer])}
</section>
`
}
```
