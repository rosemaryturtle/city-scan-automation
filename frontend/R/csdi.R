...
common_plot +
  geom_line(data = csdi_projections, aes(y = median, color = ssp)) +
  labs(
    title = "Projected duration of cold spells",
    x = "Year",
    y = "Days in a row",
    fill = "Scenario", color = "Scenario",
    caption = break_lines("Number of days when temperature is below base period's 90th percentile min temperature for that calendar day, day before, and day after", 80, "\n"),
  ) +
  annotate("text",
    x = as.Date("2022-07-01"),
    y = annotation_height(filter(csdi_all_df, Likelihood %in% c("Observed", "median"))$value, lower_limit = 0),
    label = "← Observed",
    hjust = 1) +
  annotate("text",
...
common_plot +
  geom_ribbon(data = filter(csdi_projections, ssp == selected_ssp), aes(ymin = p10, ymax = p90, fill = ssp), alpha = 0.33) +
  geom_line(data = filter(csdi_projections, ssp == selected_ssp), aes(y = median, color = ssp)) +
  labs(
    title = paste("Projected duration of cold spells under", selected_ssp),
    x = "Year",
    y = "Days in a row",
    fill = "Scenario", color = "Scenario",
    caption = break_lines("Number of days when temperature is below base period's 90th percentile min temperature for that calendar day, day before, and day after.\nShaded area shows 10th to 90th percentile cases", 80, "\n"),
  ) +
  annotate("text",
    x = as.Date("2022-07-01"),
    y = annotation_height(filter(csdi_all_df, ssp == selected_ssp)$value, lower_limit = 0),
    label = "← Observed",
    hjust = 1) +
  annotate("text",
    x = as.Date("2024-07-01"),
...
ggsave(file.path(charts_dir, paste0("csdi-", selected_ssp, ".png")), width = 6, height = 4)
