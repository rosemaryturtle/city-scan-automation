...
for (zone in zones){
  tryCatch({
    ...
    p <- ggplot() +
      ...
      geom_spatraster(data = kde, na.rm = TRUE, alpha = 0.85) +
      ...
      scale_fill_gradientn(
        name = paste0(
          ...
          "kernel density estimation</i><br>"
        ),
        colours = RedOr,
        na.value = NA,
    ...
    save_plot(
      plot = p,
      filename = paste0(zone, "_acitivity_kde.png"),
    ...
    message(paste("Success:", zone, "economic activity KDE"))
  }, error = function(e) {
    message(paste("Failure:", zone, "economic_activity_kde -", e$message))
  })
}
