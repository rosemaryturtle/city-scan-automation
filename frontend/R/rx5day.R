...
# # Y as base return period, X as new return period
# ggplot(changefactor, aes(y = base_return_period, x = new_return_period, color = ssp)) +
#   # geom_line(aes(group = period), color = "black", alpha = 0.5) +
#   geom_point(aes(shape = Likelihood)) +
#   geom_line(aes(group = base_return_period)) +
#   facet_grid(rows = vars(ssp))

# # X as time, Y as return period
# bind_rows(
...
#     adjusted_y = 1)) %>%
#   ggplot(aes(x = center, y = new_return_period)) + 
#   geom_point()
