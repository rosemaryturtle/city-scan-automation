...
ggplot(data = wsdi_projections, aes(x = date)) +
  ...
  labs(
    # title = "Days above 90th percentile daily max temperature in 1995–2014 [TK???]",
    x = "Year",
    y = "Days in a row",
    fill = "Scenario", color = "Scenario",
    caption = "Number of days in which the temperature exceeds the base period's 90th percentile max temperature for a given calendar day, and the two days on either side",
  ) +
