...
flood_exposure <- fuzzy_read(tabular_dir, "flood_wsf", read_csv)
if (inherits(flood_exposure, "data.frame")) {
  flood_exposure <- flood_exposure %>%
    ...
    rename_with(.cols = ends_with("2020"), \(x) str_replace(x, "_2020", "")) %>%
  ...
  plot_flood_exposure <- function(flood_type) {
    ...
    exposure_plot <- flood_exposure %>%
      ...
      geom_point(data = \(d) filter(d, type != "combined"), size = 1) +
      scale_x_continuous(
