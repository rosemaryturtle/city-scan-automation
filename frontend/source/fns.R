...
prepare_parameters <- function(yaml_key, ...) {
  ...
  params$palette <- sapply(params$palette, \(p) {
    ...
    if (nchar(p) == 7 | substr(p, 1, 1) != "#") {
      return(alpha(p, layer_alpha))
    }
...
create_layer_function <- function(data, yaml_key = NULL, params = NULL, color_scale = NULL, message = F, fuzzy_string = NULL, ...) {
  ...
  if (params$bins > 0 && is.null(params$breaks)) {
    params$breaks <- break_pretty2(
      ...
      method = params$breaks_method %>%
        {
          if (is.null(.)) "quantile" else .
        }
    )
  }
...
plot_static <- function(data, yaml_key, filename = NULL, baseplot = NULL, ...) {
  ...
  baseplot <- if (is.null(baseplot)) {
    ...
    baseplot + ggnewscale::new_scale_fill()
  }
  p <- baseplot +
    layer +
    coord_sf(
...
get_layer_values <- function(data) {
  if (class(data)[1] %in% c("SpatRaster")) {
    ...
    stop("Data is not of class SpatRaster, SpatVector or sf")
  }
  return(values)
}
...
aspect_buffer <- function(x, aspect_ratio, buffer_percent = 0) {
  bounds_proj <- st_transform(st_as_sfc(st_bbox(x)),
    crs =
      "EPSG:3857"
  )
  ...
  long_distance <- max(c(
    ...
    st_distance(
      ...
      st_point(st_bbox(bounds_proj)[c("xmax", "ymax")])
    )[1]
  ))
  lat_distance <- max(c(
    st_distance(
  ...
  new_bounds_proj <-
    c(
      center_proj[, "X"] + (c(xmin = -1, xmax = 1) * long_distance / 2 * (1 + buffer_percent)),
